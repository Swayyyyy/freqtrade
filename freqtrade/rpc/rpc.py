...
class RPC:
    ...
    def _rpc_stats(self) -> dict[str, Any]:
        ...
        for trade in trades:
            ...
            if trade.close_date is not None and trade.open_date is not None:
                trade_dur = (trade.close_date -
                             trade.open_date).total_seconds()
    ...
    def _rpc_trade_statistics(
        ...
        winrate = (winning_trades /
                   closed_trade_count) if closed_trade_count > 0 else 0
        ...
        bot_start = KeyValueStore.get_datetime_value(
            KeyStoreKeys.BOT_START_TIME)
        return {
    ...
    def _rpc_balance(self, stake_currency: str, fiat_display_currency: str) -> dict:
        ...
        for symbol, position in self._freqtrade.wallets.get_all_positions(
        ).items():
        ...
        with self._freqtrade._exit_lock:
            ...
            for open_order in trade.open_orders:
                try:
                    ...
                    logger.info(f"Cannot query order for {trade} due to {e}.",
                                exc_info=True)
    ...
    def _rpc_delete(self, trade_id: int) -> dict[str, str | int]:
        ...
        with self._freqtrade._exit_lock:
            c_count = 0
            trade = Trade.get_trades(
                trade_filter=[Trade.id == trade_id]).first()
            if not trade:
    ...
    def _rpc_delete_lock(
        ...
        if lockid:
            locks = PairLock.session.scalars(
                select(PairLock).filter(PairLock.id == lockid)).all()
